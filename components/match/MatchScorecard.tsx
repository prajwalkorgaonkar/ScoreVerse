...
export default function MatchScorecard({ match, innings, balls, role }: Props) {
  ...
  inningsBalls.forEach(ball => {
    ...
    if (ball.is_wicket) {
      ...
      batsmanMap[bid].dismissal =
        ball.wicket_type?.replace('_', ' ') || 'out'
    }
  })
  ...
  const battingTeam =
    match.team1_id === currentInnings?.batting_team_id
      ? match.team1
      : match.team2

  const bowlingTeam =
    match.team1_id === currentInnings?.bowling_team_id
      ? match.team1
      : match.team2
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2 border rounded text-sm hover:bg-gray-800 transition"
          >
            <Copy size={14} />
          </button>
        </div>
