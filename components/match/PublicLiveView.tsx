...
export default function PublicLiveView({ match, currentInnings: initInnings, innings1: initI1, innings2: initI2, allBalls: initBalls, token }: Props) {
  ...
  // Advanced batting algorithm
  const batsmanMap: Record<string, any> = {}
  activeInningsBalls.sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()).forEach((ball: any) => {
    ...
    const isLegalForBatsman = !ball.extra_type || ball.extra_type === 'no_ball'
    if (isLegalForBatsman) {
      batsmanMap[bid].balls++
      if (ball.runs === 0) batsmanMap[bid].dots++
      else if (ball.runs === 1) batsmanMap[bid].ones++
      else if (ball.runs === 2) batsmanMap[bid].twos++
      else if (ball.runs === 3) batsmanMap[bid].threes++
      else if (ball.runs === 4) batsmanMap[bid].fours++
      else if (ball.runs === 6) batsmanMap[bid].sixes++
      
      batsmanMap[bid].runs += ball.runs
    } else if (ball.extra_type === 'wide') {
      // Wides don't count as balls faced for batsman
    }
  ...
  // Advanced bowling algorithm
  const bowlerMap: Record<string, any> = {}
  const bowlerOvers: Record<string, Record<number, { runs: number, isMaiden: boolean }>> = {}
  ...
  activeInningsBalls.forEach((ball: any) => {
    ...
    if (!bowlerOvers[wid][ov]) {
      bowlerOvers[wid][ov] = { runs: 0, isMaiden: true }
    }

    const runsConceded = (ball.runs || 0) + (ball.extra_type === 'wide' || ball.extra_type === 'no_ball' ? (ball.extras || 0) : 0)
    bowlerMap[wid].runs += runsConceded
    bowlerOvers[wid][ov].runs += runsConceded
    
    if (runsConceded > 0) {
      bowlerOvers[wid][ov].isMaiden = false
    }

    if (!ball.extra_type || ball.extra_type === 'bye' || ball.extra_type === 'leg_bye') {
      bowlerMap[wid].balls++
      if (ball.runs === 0) bowlerMap[wid].dots++
    }
    ...
    if (ball.is_wicket && ['bowled', 'caught', 'lbw', 'stumped', 'hit_wicket'].includes(ball.wicket_type)) {
      bowlerMap[wid].wickets++
    }
  })

  // Calculate maidens (only for completed overs)
  Object.keys(bowlerOvers).forEach(wid => {
    Object.entries(bowlerOvers[wid]).forEach(([ov, data]) => {
      // An over is usually 6 balls, but we check if it was a maiden
      // Strictly, it's a maiden only if 0 runs were conceded and it's a "full" over or completed.
      // For simplicity, we count any over where runsConceded was 0 as a candidate.
      if (data.isMaiden) {
        // Only count if at least 6 legal balls were bowled in that over (approximate check)
        const overBalls = activeInningsBalls.filter(b => b.bowler_id === wid && b.over_number === parseInt(ov))
        const legalBalls = overBalls.filter(b => !b.extra_type || b.extra_type === 'bye' || b.extra_type === 'leg_bye').length
        if (legalBalls === 6) {
          bowlerMap[wid].maidens++
        }
      }
    })
  })
  ...
  const getOverSummaries = (inningBalls: any[]) => {
  ...
  const activeOverSummaries = activeInnings ? getOverSummaries(activeInningsBalls) : []
  ...
  useEffect(() => {
    const channel = supabase.channel(`public-${token}`)
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'innings', filter: `match_id=eq.${match.id}` },
        (payload: any) => {
          const updatedInnings = payload.new
          if (updatedInnings.innings_number === 1) setInnings1(updatedInnings)
          if (updatedInnings.innings_number === 2) setInnings2(updatedInnings)
          
          setInnings(prev => {
            if (prev?.innings_number === updatedInnings.innings_number) return updatedInnings
            return prev
          })
        }
      )
  ...
  return (
    <div className="min-h-screen bg-arena-dark">
      <div className="max-w-2xl mx-auto px-4 pt-28 pb-6 space-y-4">
        ...
        <div className="sticky top-16 left-0 right-0 z-30 lg:hidden -mx-4 px-4 py-3 bg-arena-dark/90 backdrop-blur-xl border-b border-arena-border shadow-xl transition-all duration-300">
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <div className="flex flex-col">
                    <span className="text-[8px] font-bold text-gray-500 uppercase tracking-tighter">{team1?.short_name}</span>
                    <span className="text-lg font-display text-white leading-none">
                       {match.innings?.find((i:any)=>i.batting_team_id===team1?.id)?.total_runs || 0}/{match.innings?.find((i:any)=>i.batting_team_id===team1?.id)?.total_wickets || 0}
                    </span>
                 </div>
                 <span className="text-[10px] text-gray-700 font-display">VS</span>
                 <div className="flex flex-col text-right">
                    <span className="text-[8px] font-bold text-gray-500 uppercase tracking-tighter">{team2?.short_name}</span>
                    <span className="text-lg font-display text-white leading-none">
                       {match.innings?.find((i:any)=>i.batting_team_id===team2?.id)?.total_runs || 0}/{match.innings?.find((i:any)=>i.batting_team_id===team2?.id)?.total_wickets || 0}
                    </span>
                 </div>
              </div>
              <div className="h-8 w-px bg-arena-border" />
              <div className="flex flex-col items-end">
                 <span className="text-[8px] font-bold text-pitch-400 uppercase tracking-widest">{innings?.batting_team_id === team1?.id ? team1?.short_name : team2?.short_name}</span>
                 <span className="text-xs text-gray-500 font-mono">({innings?.total_overs}.{innings?.total_balls} ov)</span>
              </div>
           </div>
        </div>
        ...
        {/* Info & About Section */}        <AnimatePresence mode="wait">
          {activeTab === 'info' ? (
            <motion.div key="info" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4 pt-4">
              {match.tournament && (
            ...
            <motion.div key="scorecard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4 pt-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="glass-card rounded-2xl overflow-hidden shadow-lg border border-arena-border/50">
                    <div className="px-5 py-4 border-b border-arena-border/50 bg-arena-dark/40 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         <div className="w-3 h-3 rounded-full" style={{ backgroundColor: activeBattingTeam?.color }} />
                         <h3 className="text-sm font-bold text-white uppercase tracking-wider">Batting — {activeBattingTeam?.name}</h3>
                      </div>
                    </div>
                      ...
                      <table className="arena-table w-full whitespace-nowrap">
                        <thead>
                          <tr>
                            ...
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">R</th>
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">B</th>
                            <th className="hidden lg:table-cell text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-8">0s</th>
                            <th className="hidden xl:table-cell text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-8">1s</th>
                            <th className="hidden xl:table-cell text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-8">2s</th>
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">4s</th>
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">6s</th>
                        ...
                        <tbody>
                          {Object.values(batsmanMap).map((b: any) => (
                            <tr key={b.id} className="border-t border-arena-border/30 hover:bg-white/5 transition-colors">
                              ...
                              <td className="py-3 text-gray-500 text-[10px] sm:text-xs capitalize">{b.out ? b.dismissal : <span className="text-pitch-400 font-medium">not out</span>}</td>
                              ...
                              <td className="hidden lg:table-cell text-center py-3 text-gray-600 text-xs">{b.dots}</td>
                              <td className="hidden xl:table-cell text-center py-3 text-gray-500 text-xs">{b.ones}</td>
                              <td className="hidden xl:table-cell text-center py-3 text-gray-500 text-xs">{b.twos}</td>
                              <td className="text-center py-3 text-pitch-400 text-sm font-semibold">{b.fours}</td>
                              <td className="text-center py-3 text-pitch-300 text-sm font-semibold">{b.sixes}</td>
                              <td className="pr-5 text-center py-3 text-gray-400 font-mono text-xs font-semibold">
                                {b.balls > 0 ? ((b.runs / b.balls) * 100).toFixed(1) : '-'}
                              </td>
                            </tr>
                          ))}
                          {Object.keys(batsmanMap).length === 0 && (
                            <tr><td colSpan={10} className="text-center py-6 text-gray-500 italic text-sm">No batters recorded yet</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div className="glass-card rounded-2xl overflow-hidden shadow-lg border border-arena-border/50">
                    <div className="px-5 py-4 border-b border-arena-border/50 bg-arena-dark/40 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         <div className="w-3 h-3 rounded-full" style={{ backgroundColor: activeBowlingTeam?.color }} />
                         <h3 className="text-sm font-bold text-white uppercase tracking-wider">Bowling — {activeBowlingTeam?.name}</h3>
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="arena-table w-full whitespace-nowrap">
                        <thead>
                          <tr>
                            ...
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">M</th>
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">R</th>
                            <th className="text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">W</th>
                            <th className="hidden lg:table-cell text-center text-xs text-gray-400 uppercase tracking-widest font-semibold w-10">Dots</th>
                          ...
                          {Object.values(bowlerMap).map((b: any) => {
                            ...
                            return (
                              <tr key={b.id} className="border-t border-arena-border/30 hover:bg-white/5 transition-colors">
                                ...
                                <td className="text-center py-3 text-pitch-400 font-mono text-sm font-bold">{b.maidens}</td>
                                ...
                                <td className="hidden lg:table-cell text-center py-3 text-gray-600 text-xs font-mono">{b.dots}</td>
                  ...
                  {/* Over Histories */}
                  <div className="glass-card rounded-2xl overflow-hidden shadow-lg border border-arena-border/50">
                    <div className="px-5 py-4 border-b border-arena-border/50 bg-arena-dark/40">
                       <h3 className="text-sm font-bold text-white uppercase tracking-wider">Over-by-Over History</h3>
                    </div>
                    <div className="divide-y divide-arena-border/30">
                      {activeOverSummaries.map((over) => (
                        <div key={over.number} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-arena-dark border border-arena-border flex items-center justify-center font-bold text-gray-300">
                              {over.number + 1}
                            </div>
                            <div className="flex gap-1.5 flex-wrap max-w-[200px] sm:max-w-none">
                              {[...over.balls].reverse().map((ball: any) => (
                                <div key={ball.id} className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${getBallColor(ball)}`}>
                                  {getBallLabel(ball)}
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="text-right">
                             <div className="text-white font-bold">{over.runs} Runs</div>
                             <div className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">
                                {over.wickets > 0 ? `${over.wickets} Wicket${over.wickets > 1 ? 's' : ''}` : 'No Wickets'}
                             </div>
                          </div>
                        </div>
                      ))}
                      {activeOverSummaries.length === 0 && (
                         <div className="p-8 text-center text-gray-600 text-sm">Waiting for first over to be completed...</div>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="glass-card rounded-xl p-5 mt-6 border border-arena-border/30">
          <div className="flex items-center justify-between mb-3">
             <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Recent Balls</h3>
             <span className="text-[10px] text-gray-600">LATEST FIRST</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {balls.slice(0, 12).map((ball: any) => (
              <motion.div
                key={ball.id}
